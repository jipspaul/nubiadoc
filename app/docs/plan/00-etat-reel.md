# État réel du code — inventaire (2026-06-09)
...
| **M5 Wedge — signature** | `SignatureBloc` + bodies pending/result ; `signature_repository` (domain) | 🟡 coquille signature seulement |
| **M5 Wedge — devis/paiement** | `Quote` (entité) + `billing_repository` (port) | 🔲 data billing vide, pages devis + paiement Stripe absentes |
...
- `lib/data/remote/billing/` ne contient qu'un `.gitkeep`.
- Dépendances paiement (`flutter_stripe`/`pay`) et GenUI (`flutter_genui`/`firebase_ai`) absentes du `pubspec.yaml`.
