...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  React.useLayoutEffect(() => {
    ...
    // Self-heal: if the restored (or default) camera leaves NO artboard on
    // screen — e.g. content grew/shrank since the camera was saved and it now
    // points into empty space — snap back to the top-left of the content so
    // the canvas never appears blank. Runs once after children have mounted.
    const healT = setTimeout(() => {
      const vp = vpRef.current;
      if (!vp) return;
      const vr = vp.getBoundingClientRect();
      const slots = vp.querySelectorAll('[data-dc-slot], [data-dc-section]');
      if (!slots.length) return;
      let visible = false;
      slots.forEach((s) => {
        const r = s.getBoundingClientRect();
        if (r.right > vr.left + 24 && r.left < vr.right - 24 && r.bottom > vr.top + 24 && r.top < vr.bottom - 24) visible = true;
      });
      if (!visible) {
        tf.current = { x: 64, y: 72, scale: Math.min(maxScale, Math.max(minScale, 0.55)) };
        apply();
      }
    }, 420);
    ...
    return () => { clearTimeout(healT); window.removeEventListener('pagehide', flush); flush(); };
  }, []);
