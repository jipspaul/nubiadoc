 ...
 *
 * Scénarios :
 *   1. login secrétaire → inviter membre (POST /v1/cabinet/members 201)
 *      → modifier rôle (PATCH /v1/cabinet/members/:user_id 200)
 *      → retirer membre (DELETE /v1/cabinet/members/:user_id 204 ou 200)
...
// ─────────────────────────────────────────────────────────────────────────────
// Scénario 1 : Inviter → modifier → retirer un membre
// ─────────────────────────────────────────────────────────────────────────────
test('secrétaire : inviter (201) → modifier rôle (200) → retirer (200/204) un membre', async ({ page }) => {
  const memberEmail = freshEmail();

  // ── 1. Connexion secrétaire ───────────────────────────────────────────────
  await loginAs(page, 'secretary');

  // ── 2. Inviter un membre via l'API (POST /v1/cabinet/members → 201) ───────
  const { inviteStatus, userId } = await page.evaluate(
    async ({ apiBase, email }: { apiBase: string; email: string }) => {
      ...
      const resp = await fetch(`${apiBase}/v1/cabinet/members`, {
        ...
        body: JSON.stringify({ email, role: 'secretary' }),
      });
      const data = resp.ok
  ...
  // Invitation réussie (201) ou doublon accepté (200)
  expect(inviteStatus, `POST /v1/cabinet/members attendu 201, reçu ${inviteStatus}`).toBeLessThan(300);
  expect(userId).toBeTruthy();

  // ── 3. Modifier le rôle du membre (PATCH /v1/cabinet/members/:id → 200) ───
  ...
  // ── 4. Retirer le membre (DELETE /v1/cabinet/members/:id → 200 ou 204) ────
...
test('secrétaire : GET /v1/cabinet/quotes → 200 et page /secretary/facturation visible', async ({ page }) => {
  ...
  await expect(page.locator('h1')).toBeVisible({ timeout: 10_000 });
  await expect(page.locator('#quotes-status, #quotes-table, #quotes-empty')).toBeVisible({
    timeout: 10_000,
  });
});
