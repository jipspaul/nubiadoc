...
test('le formulaire /scheduling/appointment-preparation est visible avec les champs requis', async ({ page }) => {
  await page.goto('/scheduling/appointment-preparation');
...
test('happy path => GET preparation retourne 200, bring et adresse visibles', async ({ page }) => {
  await page.route('**/v1/appointments/*/preparation', (route) => {
    route.fulfill({
      ...
      body: JSON.stringify({
        ...
        reminder_at: '2026-06-01T08:00:00Z',
      }),
    });
  });
