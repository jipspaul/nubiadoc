...
test('la page /test/messaging/conversation-messages affiche le formulaire avec les champs requis', async ({ page }) => {
  ...
  await expect(page.locator('input[name="page"]')).toBeVisible();
...
test('GET 422 — page invalide affiché dans #result', async ({ page }) => {
  await page.route('**/v1/conversations/*/messages**', (route) => {
    route.fulfill({
      status: 422,
      contentType: 'application/json',
      body: JSON.stringify({ code: 'validation_error', fields: { page: 'must be a positive integer' } }),
    });
  });
  ...
  await page.locator('input[name="access_token"]').fill('fake-access-token');
  await page.locator('input[name="conversation_id"]').fill('00000000-0000-0000-0000-000000000001');
  await page.locator('input[name="page"]').fill('abc');
  await page.getByRole('button', { name: /^GET$/i }).click();
