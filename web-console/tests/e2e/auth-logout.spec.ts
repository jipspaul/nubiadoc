...
test('le formulaire /auth/logout est visible avec les champs et le bouton', async ({ page }) => {
  await page.goto('/test/auth/logout');
...
test('submit avec credentials bidon affiche un résultat (status visible)', async ({ page }) => {
  ...
  await expect(page).toHaveURL('/test/auth/logout');
});
