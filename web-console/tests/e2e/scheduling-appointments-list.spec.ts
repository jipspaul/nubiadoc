...
test('le formulaire /scheduling/appointments-list est visible avec les champs requis', async ({ page }) => {
  await page.goto('/scheduling/appointments-list');
...
test('submit avec token bidon affiche un résultat (status visible)', async ({ page }) => {
  ...
  await expect(page).toHaveURL('/scheduling/appointments-list');
});
