...
test('le formulaire /test/treatment-plans/list est visible avec les champs requis', async ({ page }) => {
  await page.goto('/test/treatment-plans/list');
...
test('submit avec token bidon affiche un résultat (status visible)', async ({ page }) => {
  ...
  await expect(page).toHaveURL('/test/treatment-plans/list');
});
