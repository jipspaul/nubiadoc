...
test('le formulaire /scheduling/appointment-detail est visible avec les champs requis', async ({ page }) => {
  await page.goto('/scheduling/appointment-detail');
...
test('ID valide => affiche HTTP 200 ou erreur réseau (pas de backend en CI)', async ({ page }) => {
  ...
  await expect(page).toHaveURL('/scheduling/appointment-detail');
});
