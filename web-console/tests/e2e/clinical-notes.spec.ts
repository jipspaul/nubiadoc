...
const PAGE_URL = '/clinical/patients/p-test-1/notes';

test('la page /clinical/patients/{id}/notes affiche les sections GET et POST', async ({ page }) => {
...
test('GET /v1/cabinet/patients/{id}/notes praticien → 200 timeline affichée', async ({ page }) => {
...
test('POST /v1/cabinet/patients/{id}/notes → 201 note créée affichée', async ({ page }) => {
  await page.route('**/v1/cabinet/patients/*/notes', (route) => {
    if (route.request().method() !== 'POST') return route.fallback();
    route.fulfill({
      status: 201,
      contentType: 'application/json',
      body: JSON.stringify({ id: 'note-3', note_kind: 'observation', text: 'Suivi post-op', created_at: '2026-06-07T11:00:00Z' }),
    });
  });
  ...
  await page.locator('#form-post textarea[name="text"]').fill('Suivi post-op');
  ...
  await expect(page.locator('#result-post')).toContainText('note-3');
});

test('GET secretary → 403 affiché dans result-get', async ({ page }) => {
  await page.route('**/v1/cabinet/patients/*/notes', (route) => {
    if (route.request().method() !== 'GET') return route.fallback();
    route.fulfill({
      status: 403,
