// Regression test for BUG-1: concurrent 401s dropped during token refresh.
// auth_interceptor.dart used to complete concurrent 401s with handler.next(err)
// while _isRefreshing was true. The fix uses a Completer<void>? queue so that
// all waiting 401s retry their own request once the shared refresh completes.
import 'dart:async';
...
void main() {
  test('concurrent 401s should all be retried after a successful refresh',
    ...
    expect(
      ...
      3,
      reason:
          'concurrent 401s were dropped instead of retried (regression: BUG-1)',
    );
  });
}
