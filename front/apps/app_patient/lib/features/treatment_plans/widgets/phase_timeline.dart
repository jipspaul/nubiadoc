...
/// Frise verticale des phases d'un plan de soins (#5296) : pastilles +
/// connecteurs, dans l'ordre `position`. Chrome uniquement — le contenu
/// métier de chaque étape (statut, montants, CTA…) reste porté par
/// `treatment_plan_detail_page.dart` via [PhaseStep.card]. Maquette :
...
/// Ligne `.stp` d'une phase : colonne gauche `.rl` (pastille + connecteur),
/// colonne droite `.bd` (carte de phase fournie par l'appelant).
class PhaseStep extends StatelessWidget {
  ...
  /// terminée (1-based, ordre d'affichage `position`).
  final int number;
                ...
                if (!isLast)
                  Expanded(
                    child: Container(
                      width: 2,
                      color:
                          _isDone ? NubiaColors.brand200 : NubiaColors.n300,
                    ),
                  ),
              ],
            ),
...
/// Pastille `.dot` d'une étape — trois traitements distincts (#5296) :
/// « terminée » plein `--brand600` + `check`, « en cours » anneau
/// `--n900` 2,5px + numéro, « à venir » bordure `--n300` + numéro `--n500`.
