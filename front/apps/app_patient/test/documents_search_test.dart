...
void main() {
  ...
  testWidgets('champ de recherche affiche le placeholder verbatim',
      (tester) async {
    ...
    expect(find.text('Rechercher un document…'), findsOneWidget);
  });

  testWidgets('saisir un terme filtre par nom, insensible à la casse',
      (tester) async {
    await tester.pumpWidget(
      MaterialApp(
        theme: NubiaTheme.light,
    ...
    expect(find.byKey(const Key('document_p1')), findsNothing);
    expect(find.byKey(const Key('document_m1')), findsOneWidget);
    expect(find.byKey(const Key('document_o1')), findsNothing);
  });

  testWidgets('recherche combinée au filtre catégorie actif', (tester) async {
    await tester.pumpWidget(
      MaterialApp(
        theme: NubiaTheme.light,
        home: const Scaffold(body: DocumentsPage()),
  ...
  testWidgets('vider le champ restaure la liste filtrée par catégorie seule',
      (tester) async {
    await tester.pumpWidget(
      MaterialApp(
        theme: NubiaTheme.light,
    ...
    await tester.enterText(find.byKey(const Key('documents_search')), '');
    await tester.pumpAndSettle();
