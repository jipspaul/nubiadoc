...
| POST | `/v1/auth/mfa/disable` | — | Redemande `email`+mot de passe (pas de token, cf. #7342), désactive la MFA. |
...
`POST /v1/auth/mfa/disable` — **sans token** (#7342), body : `email`, `password` (mot de passe **courant**, redemandé pour confirmer — c'est la preuve de possession). → `200 { message:"MFA désactivée." }` : supprime l'enrôlement `mfa_enrollment`, `app_user.totp_enabled = false`, le login suivant n'exige plus `mfa_code`. Seule voie de désactivation hors reset de mot de passe complet (#7328 : avant cette route, activer la MFA via `/mfa/verify` était irréversible sans passer par `POST /v1/auth/password/reset`, qui exige l'accès à la boîte mail et change le mot de passe au passage — inutilisable sur un compte partagé). Route délibérément anonyme (#7342, suite de #7328) : exiger un token pro fermait un cycle impossible à rompre puisque `login` refuse justement d'en émettre un tant que la MFA est active — c'était le seul moyen d'atteindre la route. Mêmes gardes anti-abus que `login` : rate-limit IP + email, réponse `401 unauthenticated` neutre (mot de passe faux **ou** email inconnu, anti-énumération). Erreurs : `401 unauthenticated`, `409 invalid_status` (MFA déjà désactivée), `429 too_many_requests`.
