...
### ADR-012 — Interopérabilité FHIR R4 + HL7 v2, auth partenaire OAuth2 client-credentials
- **Décision** : exposer une surface d'interopérabilité façon Doctolib (synchro agenda, annuaire, référentiel patient) sous forme d'une API REST **FHIR R4** montée sous `/v1/interop/fhir/...` (le préfixe `/v1` respecte la règle « pas d'API non versionnée » tout en publiant une base FHIR conforme via le `CapabilityStatement`), plus un listener **HL7 v2 natif en MLLP** (port dédié, second `tokio::task` dans le même binaire — pas un second conteneur, cohérent avec le monolithe modulaire d'ADR-002). Les deux surfaces convergent vers la même couche de service (`api/src/interop/*` pour le REST, `api/src/hl7v2/dispatch.rs` pour le MLLP), afin d'éviter que la logique métier diverge entre les deux protocoles.
- **Authentification partenaire** : deux mécanismes distincts selon le protocole, volontairement non unifiés — OAuth2 *client-credentials* (`POST /v1/interop/oauth/token`, nouvelle audience JWT `aud:"interop"`, scopes fins) pour le REST FHIR ; certificat client **mTLS** (empreinte SHA-256, pas le CN) pour le MLLP, un partenaire HL7v2 n'ayant pas de notion de « requête » individuelle à authentifier. Les deux registres de partenaires (`interop_client`/`interop_client_secret` d'un côté, `hl7v2_partner`/`hl7v2_partner_facility_map` de l'autre) sont délibérément des tables distinctes malgré leur rôle similaire, pour ne jamais confondre un consommateur REST et un émetteur HL7v2 dans les logs d'audit.
