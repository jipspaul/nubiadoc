...
pub async fn download_cabinet_patient_document(
    ...
    if claims.role == "secretary" {
        let in_scope = match claims.secretariat_id {
            // #5533 : même garde que list_patient_documents/upload_patient_document
            // (#5509) — un patient walk-in (quick_create_patient) n'a aucun
            // appointment à la création, d'où le OR sur created_by_secretariat_id
            // (posé à la création).
