...
//! déclinée ou a expiré sans réponse — #5730.
//!
//! Quoi : avant ce module, `visit_request.status` restait bloqué à `offered`
...
//!
//! Deux points d'entrée pour cette résolution, tous deux SQL (migration 0236) —
...
/// n'ayant jamais répondu), puis résout chaque demande concernée. Ne panique
/// jamais sur une demande individuelle en erreur : le balayage continue.
///
/// La résolution SQL (`expire_stale_visit_offers`, migration 0236) a déjà
/// committé au moment où [`apply_settlement`] s'exécute pour chaque ligne —
/// une erreur ici (typiquement `notify_nurse_staff`/`notify_patient_account`)
/// ne remet PAS la demande en jeu : elle est déjà sortie de `offered`, donc
/// ne réapparaîtra plus jamais dans un passage suivant. Seule la
/// notification applicative (in-app/push) peut être perdue sur cet aléa, pas
/// la résolution elle-même (déjà garantie par le passage SQL qui précède).
