//! Tests d'intégration : POST /v1/cabinet/appointments/:id/start
...
const JWT_SECRET: &str = "test-secret-appt-start";
...
/// Insère les fixtures minimales : cabinet + praticien + patient + appointment.
...
async fn insert_fixture(db: &PgPool, status: &str) -> (Uuid, Uuid, Uuid, Uuid) {
    ...
    .bind(prac_user_id)
    .bind(format!("start-prac+{}@nubia.test", prac_user_id))
    .execute(&mut *tx)
    .await
    .unwrap();

    sqlx::query(
        "INSERT INTO cabinet (id, raison_sociale, specialite) VALUES ($1, 'Cabinet Start Test', 'dentaire')",
    )
    .bind(cabinet_id)
    .execute(&mut *tx)
    .await
...
// ── Test 1 : praticien, RDV confirmed → 200 + status in_progress ─────────────

#[tokio::test]
async fn start_consultation_practitioner_confirmed_returns_200() {
    if !db_available() {
        return;
    }
...
#[tokio::test]
async fn start_consultation_secretary_returns_403() {
    if !db_available() {
        return;
    }
    ...
    // Génère un token secrétaire pour le même cabinet.
    let secretary_user_id = Uuid::new_v4();
    let token = make_secretary_token(secretary_user_id, cabinet_id);
    let response = server
        .oneshot(
            Request::builder()
                .method("POST")
...
#[tokio::test]
async fn start_consultation_invalid_status_returns_409() {
    if !db_available() {
        return;
    }
