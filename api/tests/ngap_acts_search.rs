//! Tests d'intégration : GET /v1/ngap/acts (référentiel NGAP, #4053)
...
const JWT_SECRET: &str = "test-secret-ngap-acts";
...
async fn search_by_label_returns_matches() {
    ...
    let (status, json) = get(Some(&token), "/v1/ngap/acts?q=consultation").await;

    assert_eq!(status, StatusCode::OK);
    let data = json["data"].as_array().expect("data tableau");
    assert!(!data.is_empty(), "au moins une consultation attendue");
    assert!(
    ...
    // Champs du contrat NGAP (lettre_cle/coefficient en plus de code/label).
    let first = &data[0];
    assert!(first["code"].is_string());
    assert!(first["lettre_cle"].is_string());
    assert!(first["coefficient"].is_number());
...
async fn search_by_code_returns_match() {
    ...
    let token = make_pro_token("practitioner");
    let (status, json) = get(Some(&token), "/v1/ngap/acts?q=SC13").await;

    assert_eq!(status, StatusCode::OK);
    let data = json["data"].as_array().unwrap();
    assert!(data.iter().any(|a| a["code"] == "SC13"));
}
...
async fn no_query_returns_catalog() {
    ...
    let token = make_pro_token("practitioner");
    let (status, json) = get(Some(&token), "/v1/ngap/acts").await;
...
async fn secretary_forbidden_and_anonymous_unauthorized() {
    ...
    let (s_status, _) = get(Some(&make_pro_token("secretary")), "/v1/ngap/acts").await;
    assert_eq!(s_status, StatusCode::FORBIDDEN);

    let (a_status, _) = get(None, "/v1/ngap/acts").await;
