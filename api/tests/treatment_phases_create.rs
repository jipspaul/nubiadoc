...
async fn insert_practitioner_without_care_relationship(db: &PgPool, cabinet_id: Uuid) -> Uuid {
    ...
    .bind(user_id)
    .bind(format!("tph-prac-norel+{}@nubia.test", user_id))
    .execute(db)
    .await
    .unwrap();
...
// ── Test 8 : praticien sans relation de soin avec le patient → 403 (#4400) ───

#[tokio::test]
async fn create_treatment_phase_no_care_relationship_returns_403() {
    if !db_available() {
        return;
    }
    let db = owner_pool().await;
    ...
    let resp = app(make_state(app_pool().await))
        .oneshot(
            Request::builder()
                ...
                .header("Authorization", format!("Bearer {}", token))
                .body(Body::from(
                    json!({ "title": "Phase", "position": 0 }).to_string(),
                ))
                .unwrap(),
        )
        .await
