//! Tests d'intégration : PATCH /v1/cabinet/provider
...
// ── Test 1 : happy path — admin → 200 avec champs mis à jour ─────────────────

#[tokio::test]
async fn patch_provider_happy_path_returns_200() {
    if !db_available() {
        return;
    }
    let email = format!("patch_provider_{}@test.local", Uuid::new_v4());
    let db = app_pool().await;
    let (token, _, _) = register_pro(db.clone(), &email).await;

    let body = json!({
        "bio": "Expert en implantologie.",
    ...
    let bytes = axum::body::to_bytes(resp.into_body(), usize::MAX)
        ...
        .unwrap();
    let v: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
    assert_eq!(v["bio"], "Expert en implantologie.");
    assert_eq!(v["specialite"], "implantologie");
    assert_eq!(v["langues"], json!(["fr", "en"]));
    assert_eq!(v["pmr"], true);
    assert_eq!(v["is_listed"], false);
    assert!(v["id"].is_string(), "id manquant");
...
// ── Test 2 : secretary → 403 ─────────────────────────────────────────────────

#[tokio::test]
async fn patch_provider_secretary_returns_403() {
    if !db_available() {
        return;
    }
    ...
    let resp = app(make_state(db))
        .oneshot(
            Request::builder()
                ...
                .body(Body::from(json!({"bio": "Test"}).to_string()))
                .unwrap(),
        )
        .await
        .unwrap();
