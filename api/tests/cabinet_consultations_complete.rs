...
async fn complete_consultation_with_acts_returns_200_and_invoice() {
    ...
    // Vérifie que completed_at est posé sur la séance.
    let session_check = sqlx::query(
        "SELECT completed_at IS NOT NULL AS has_completed_at \
         FROM consultation_session WHERE id = $1",
    )
    .bind(session_id)
    .fetch_one(&db)
    .await
...
#[tokio::test]
async fn complete_consultation_other_practitioner_returns_403() {
    if !db_available() {
        return;
    }
    let db = owner_pool().await;
    ...
    // Second praticien dans le même cabinet — pas propriétaire de la séance.
    ...
    let other_prac_id = Uuid::new_v4();
    {
        ...
        .bind(format!("other-complete+{}@nubia.test", other_prac_user_id))
        .execute(&mut *tx)
        .await
        .unwrap();
        sqlx::query("INSERT INTO practitioner (id, cabinet_id, user_id) VALUES ($1, $2, $3)")
