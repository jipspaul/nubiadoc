...
fn make_pro_jwt_directions(user_id: Uuid, cabinet_id: Uuid) -> String {
...
/// Sans JWT → 401.
#[tokio::test]
async fn get_directions_no_jwt_returns_401() {
    ...
    let response = app(state)
        .oneshot(
            Request::builder()
                .method("GET")
                .uri(format!("/v1/appointments/{}/directions", Uuid::new_v4()))
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
/// Token pro → 403 (route réservée aux patients).
#[tokio::test]
async fn get_directions_pro_token_returns_403() {
    let db = PgPool::connect_lazy(
        &std::env::var("APP_DATABASE_URL")
            .unwrap_or_else(|_| "postgres://nubia_app@localhost:5432/nubia".into()),
    )
        ...
        .oneshot(
            Request::builder()
                ...
                .header(
                    ...
                    format!(
                        "Bearer {}",
                        make_pro_jwt_directions(Uuid::new_v4(), Uuid::new_v4())
                    ),
                )
                .body(Body::empty())
                .unwrap(),
