//! Tests d'intégration : POST /v1/appointments/:id/checkin
...
async fn insert_fixture(
    ...
    sqlx::query("INSERT INTO cabinet (id, raison_sociale, specialite) VALUES ($1, $2, 'dentaire')")
        .bind(cabinet_id)
        .bind(format!("Cabinet Checkin {}", cabinet_id))
        .execute(&mut *tx)
        .await
        .unwrap();
...
// ── Test 1 : happy path method="manual" → 200 ────────────────────────────────

#[tokio::test]
async fn post_checkin_manual_happy_path_returns_200() {
    if !db_available() {
        return;
    }
    ...
    .bind(format!("checkin-happy+{}@nubia.test", patient_user_id))
    .execute(&db)
    .await
    .unwrap();
    ...
    .bind(prac_user_id)
    .bind(format!("checkin-happy-prac+{}@nubia.test", prac_user_id))
    .execute(&db)
    .await
    .unwrap();
...
// ── Test 2 : status != confirmed → 409 {"error":"invalid_status"} ────────────

#[tokio::test]
async fn post_checkin_invalid_status_returns_409() {
    if !db_available() {
        return;
    }
    let db = seed_pool().await;
    ...
    .bind(patient_user_id)
    .bind(format!("checkin-badstatus+{}@nubia.test", patient_user_id))
    .execute(&db)
    .await
    .unwrap();
    ...
    .bind(prac_user_id)
    .bind(format!(
        "checkin-badstatus-prac+{}@nubia.test",
        prac_user_id
    ))
    .execute(&db)
    .await
    ...
    // status='requested' (pas 'confirmed') → doit retourner 409.
    let (cabinet_id, prac_id, patient_id, appt_id) =
        insert_fixture(&db, prac_user_id, patient_account_id, "requested", "now()").await;
...
async fn post_checkin_too_late_returns_422() {
    ...
    .bind(patient_user_id)
    .bind(format!("checkin-toolate+{}@nubia.test", patient_user_id))
    .execute(&db)
    .await
    .unwrap();
    ...
    .bind(prac_user_id)
    .bind(format!("checkin-toolate-prac+{}@nubia.test", prac_user_id))
    .execute(&db)
    .await
    .unwrap();

    // starts_at = il y a 2 h → hors fenêtre check-in (now > starts_at + 60 min).
...
async fn post_checkin_tomorrow_returns_422() {
    ...
    .bind(patient_user_id)
    .bind(format!("checkin-tomorrow+{}@nubia.test", patient_user_id))
    .execute(&db)
    .await
    .unwrap();
    ...
    .bind(prac_user_id)
    .bind(format!("checkin-tomorrow-prac+{}@nubia.test", prac_user_id))
    .execute(&db)
    .await
    .unwrap();
