...
// ── Test 7bis (#4404) : amount_cents absent en mode ccam_code → 422 ──────────

#[tokio::test]
async fn add_act_missing_amount_returns_422() {
    if !db_available() {
        return;
    }
    let db = owner_pool().await;
...
// ── Test 8ter (#4426) : tooth non-FDI ("banana") → 422, aucune ligne créée ────

#[tokio::test]
async fn add_act_invalid_fdi_tooth_returns_422() {
    if !db_available() {
        return;
    }
    let db = owner_pool().await;
    ...
    let count: i64 = sqlx::query(
        "SELECT count(*) AS c FROM consultation_act \
         WHERE appointment_id = $1 AND cabinet_id = $2 AND tooth = 'banana'",
    )
    .bind(appt_id)
    .bind(cabinet_id)
    .fetch_one(&mut *tx)
...
// ── Test 13 : acte invasif + flag STRUCTURÉ medico_legal.anticoagulants=true,
// SANS mot-clé en texte libre → 409 bloquant (#4403). Le flag structuré
// (#4103) était du code mort pour cette garde — seul le texte libre déclenchait.

#[tokio::test]
async fn add_act_risky_code_with_structured_anticoagulant_flag_returns_409() {
    if !db_available() {
        return;
    }
    let db = owner_pool().await;
    ...
    // Aucun mot-clé anticoagulant en texte libre — seul le flag structuré est vrai.
    let ciphertext = format!(
        "STUB_ENC:{}",
        serde_json::json!({
            "allergies": [],
            "treatments": ["Doliprane 1000mg"],
            "history": null,
            "medico_legal": {
                "ald": false,
                "anticoagulants": true,
...
// ── Test 14 : même acte re-soumis (double-submit/retry) → 409 duplicate_act,
...
// ── Test 15 : même ccam_code/amount mais TOOTH différent → 201, 201 (pas un
